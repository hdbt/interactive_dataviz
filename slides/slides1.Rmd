...
| tionButton` |                       | mericInput` | enter numbers  |
+-------------+-----------------------+-------------+----------------+
| `           | A group of check      | `ra         | A set of radio |
| checkbox G  | boxes                 | dioButtons` | buttons        |
| roup Input` |                       |             |                |
+-------------+-----------------------+-------------+----------------+
...
+-------------+-----------------------+-------------+----------------+
| `date       | A pair of calendars   | `su         | A submit       |
| RangeInput` | for selecting a date  | bmitButton` | button         |
|             | range                 |             |                |
+-------------+-----------------------+-------------+----------------+
| `fileInput` | A file upload control | `textInput` | A field to     |
|             | wizard                |             | enter text     |
+-------------+-----------------------+-------------+----------------+
| `helpText`  | Help text that can be |             |                |
|             | added to an input     |             |                |
|             | form                  |             |                |
+-------------+-----------------------+-------------+----------------+
...
-   From Markdown to interactive Slides and Documents

-   Exercises

-   R Markdown, Slides, highcharts, plotly
...
E.g. `observeEvent( input$action, print(input$name))`


* reactives are for returning a result, e.g.
`df <-  reactive( df[input$var] )` 
* observe is for side effects. In other words: Waiting for a trigger event to download, save something, validate, update and so on...
...
* `isolate()`decouples the reactive dependency taken on other reactives (like inputs, outputs, ...)
`isolate( df() )` but still reads the value of the input.
* `observeEvent(input$x, input$y)` is the same as `observe( input$x isolate(input$y))` 
* `eventReactive(input$x, input$y)` is the same as `reactive( input$x isolate (input$y))`
